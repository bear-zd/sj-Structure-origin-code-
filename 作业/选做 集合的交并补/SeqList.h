...
// 顺序表的函数成员
	...
	SeqList <ElemType>  Intersection(const SeqList <ElemType> &sa);
	SeqList <ElemType>  Union(const SeqList <ElemType> &sa);
	SeqList <ElemType>  Complement(const SeqList <ElemType> &sa);
};
...
template <class ElemType>
SeqList <ElemType> SeqList <ElemType>:: Intersection(const SeqList <ElemType> &sa)
{
    int mlength=maxLength>sa.maxLength?maxLength:sa.maxLength;
    SeqList <ElemType> sb(mlength);
    ElemType e;
    for (int i = 1; i <= GetLength(); i++)
    {
		GetElem(i, e);		// 取出la中第i个元素e
		if (sa.LocateElem(e))   // 如果e在lb中存储,将其插入到lc中
			sb.InsertElem(e);
	}
    return sb;
}
template <class ElemType>
SeqList <ElemType> SeqList <ElemType>:: Union(const SeqList <ElemType> &sa)
{
    int mlength=maxLength>sa.maxLength?maxLength:sa.maxLength;
    SeqList <ElemType> sb(*this);sb.maxLength=mlength;
    ElemType e;
    for (int i = 1; i <= sa.GetLength(); i++)
    {
		sa.GetElem(i, e);
		if (sb.LocateElem(e)==0)
			sb.InsertElem(e);
	}
...
template <class ElemType>
SeqList <ElemType> SeqList <ElemType>:: Complement(const SeqList <ElemType> &sa)
{
    int mlength=maxLength>sa.maxLength?maxLength:sa.maxLength;
    SeqList <ElemType> sb(*this);sb.maxLength=mlength;
    ElemType e;
