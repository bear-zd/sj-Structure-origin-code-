...
//  二叉树方法声明及重载编译系统默认方法声明:
    ...
    Status SortedFind(ElemType X);
    void ShowCount() const ;
};
template <class ElemType>
void BinaryTree<ElemType> ::ShowCount() const
{
...
template <class ElemType>
Status BinaryTree<ElemType> :: SortedFind(ElemType X)
{
    BinTreeNode <ElemType> *p = root,*temp ,*Nnode;
    while (p!=NULL && p->data!=X) //当p不为空或没有找到时时
    {
        temp = p; //保存现场
        if(p->data > X) //寻找适合X的点
            p=p->leftChild;
    ...
    if(p == NULL) //如果找不到
    {
        Nnode = new BinTreeNode<ElemType>(X);
        if(!temp)
            root = Nnode;
        else
        {
            if (temp->data > X)
                temp->leftChild = Nnode;
            else
                temp->rightChild = Nnode;
        }
