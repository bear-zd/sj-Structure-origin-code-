...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	GenListNode<ElemType> *Head() const;// 返回广义表的第一个元素
	...
	int GetAtom(GenListNode<ElemType > *h);
};

template <class ElemType>
GenListNode<ElemType> *GenList<ElemType>::Head() const
// 操作结果：返回广义表的第一个元素
{
	return head;
}
...
template <class ElemType>
int GenList<ElemType>::GetAtom(GenListNode<ElemType > *h)
{
    int num=0;
    for(GenListNode<ElemType> *p= h->tLink;p!=NULL;p=p->tLink)
    {
        if(p->tag==LIST)
            {num+=GetAtom(p->hLink);}
        else
