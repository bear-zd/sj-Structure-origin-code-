...
//  二叉树方法声明及重载编译系统默认方法声明:
    ...
    void AppendLsize(BinTreeNode<ElemType> *q);
    ElemType NoK(int k) const ;
};
template <class ElemType>
ElemType BinaryTree<ElemType> :: NoK(int k) const
{
    if (k < 0 || k > NodeCount() )
        {
            cout<< "错误的K值" << endl;
...
template <class ElemType>
void BinaryTree<ElemType> :: AppendLsize(BinTreeNode<ElemType> *q)
{
    	if (q != NULL)	{
                q->lsize = NodeCount(q->leftChild) + 1;
		AppendLsize(q->leftChild);	// 再遍历r的左子树
		AppendLsize(q->rightChild);	// 最后遍历r的右子树
	}
}
template <class ElemType>
...
bool BinaryTree<ElemType> :: IsSorted(BinTreeNode<ElemType> *p)
{
    ...
    {
        if(p->leftChild->data < p->data && p->rightChild->data > p->data &&IsSorted(p->leftChild) &&IsSorted(p->rightChild))
            return true;
    }
    if(p->leftChild && !p->rightChild)
    {
        if (p->leftChild->data < p->data && IsSorted(p->leftChild))
            return true;
    }
    if(!p->leftChild && p->rightChild)
    {
        if (p->rightChild->data > p->data && IsSorted(p->rightChild))
            return true;
    }
    if(!p->leftChild && !p->rightChild)
        return true;
