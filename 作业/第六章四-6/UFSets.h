...
// 并查集的函数成员:
	...
	void Union1(ElemType a, ElemType b);	 // 合并a与b所在的等价类
	void Union2(ElemType a, ElemType b);	 // 根据结点多少合并a与b所在的等价类
	void Union3(ElemType a, ElemType b);   //根据结点高度合并
    bool Differ(ElemType a, ElemType b); // 判断元素a、b是否在同一个等价类
	UFSets(const UFSets &copy);		     // 复制构造函数
	UFSets &operator =(const UFSets &copy);	// 赋值运算符
	int GetParents(int p) const;
    int Height(ElemType e) ;//求节点的高度

};
//求节点的高度
template <class ElemType>
int UFSets <ElemType> :: Height(ElemType e)
{
    int p=1; //临时值
    int Max=1;  //保存最大的高度子树
    for (int i=0;i<size;i++)
        {
            if(sets[i].parent>0&&sets[sets[i].parent].data==e)//检索，注意，当parent<0的时候程序会出有趣的错误，所以必须加这个条件
            {
            p+=Height(sets[i].data);
            if (p>Max)
                Max=p;
            p=1;
            }
...
template <class ElemType>
int UFSets<ElemType>::GetParents(int p) const
{
	if (p < 0 || p >= size)
    throw Error("双亲不存在!");			// 抛出异常
	return sets[p].parent;					// 返回双亲
}
template <class ElemType>
