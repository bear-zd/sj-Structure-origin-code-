...
// 稀疏矩阵三元组顺序表的函数成员：
	...
	TriSparseMatrix <ElemType> operator + (const  TriSparseMatrix<ElemType> &tsm2 );
};

/*template <class ElemType>
TriSparseMatrix <ElemType> TriSparseMatrix<ElemType>:: operator+ (TriSparseMatrix<ElemType> &tsm2)
{
    ElemType num1,num2;
    for(int i=0;i<rows;i++)
        for(int j=0;j<cols;j++)
    {
        this->GetElem(i,j,num1);
       tsm2.GetElem(i,j,num2);
       this->SetElem(i,j,num1+num2);
    }
...
{
    TriSparseMatrix <ElemType> c(rows,cols);
    int ir,jr,itemp;
    if(rows!=tsm.rows||cols!=tsm.cols)
    {
        cout<<"These two Matrix's rows and cols aren't equal"<<endl;
        exit(-1);
    }
    ir=jr=0;
    c.num=0;//先行后列
    while(ir<num&&jr<tsm.num)
    {
        if (triElems[ir].row==tsm.triElems[jr].row) //行相同
           {
               if(triElems[ir].col==tsm.triElems[jr].col) //列相同
               {
                   itemp=triElems[ir].value+tsm.triElems[jr].value;
                   if(itemp!=0)
                   {
                       c.triElems[c.num].row=triElems[ir].row;
                       c.triElems[c.num].col=triElems[ir].col;
                       c.triElems[c.num].value=itemp;
                       c.num++;
                   }
                   ir++;jr++;
                }
            else if(triElems[ir].col>tsm.triElems[jr].col)
                {
                  c.triElems[c.num].row=tsm.triElems[jr].row;
                  c.triElems[c.num].col=tsm.triElems[jr].col;
                  c.triElems[c.num].value=tsm.triElems[jr].value;
                  c.num++;jr++;
                }
            else if(triElems[ir].col<tsm.triElems[jr].col)
                {
                  c.triElems[c.num].row=triElems[ir].row;
                  c.triElems[c.num].col=triElems[ir].col;
                  c.triElems[c.num].value=triElems[ir].value;
                  c.num++;ir++;
                }
           }
            else if(triElems[ir].row<tsm.triElems[jr].row)
                {
                  c.triElems[c.num].row=triElems[ir].row;
                  c.triElems[c.num].col=triElems[ir].col;
                  c.triElems[c.num].value=triElems[ir].value;
