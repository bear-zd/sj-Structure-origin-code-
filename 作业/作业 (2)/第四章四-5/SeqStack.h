...
//  顺序栈的方法声明及重载编译系统默认方法声明:
	...
	Status PushS(SeqStack<ElemType> &s , const ElemType e);
	Status Top(ElemType &e) const;				    // 取顶元素
	Status Pop(ElemType &e);					    // 出栈
	Status PopS(SeqStack<ElemType> &s , ElemType &e);
...
template<class ElemType>
Status SeqStack<ElemType>::PushS(SeqStack<ElemType> &s,const ElemType e)
// 操作结果：将元素e追加到栈顶,如成功则返加SUCCESS,如栈已满将返回OVER_FLOW
{
	 return Push(e);
}
...
template <class ElemType>
Status SeqStack<ElemType>::PopS(SeqStack<ElemType> &s , ElemType &e)
{
    ElemType e1;
    if(!(IsEmpty()))
    {
