...
public:
	...
	SeqQueue<ElemType> & YHTriangle(const int n);
	Status EnQueueY(const ElemType e);				     // 入队操作
};

// 循环队列类的实现部分
template <class ElemType>
SeqQueue<ElemType> & SeqQueue <ElemType> :: YHTriangle(const int n)
{
if (n<1)
return *this;
ElemType o=0,p;EnQueueY(0);
 for(int i=1;i<n;i++,EnQueueY(0))
    {if(i==1)
        {EnQueueY(1);EnQueueY(1);continue;}
    else
    {
        for(int j=0;j<i+2;j++)
        {
...
template<class ElemType>
Status SeqQueue<ElemType>::EnQueueY(const ElemType e)
// 操作结果：如果队列已满，返回OVER_FLOW,
// 否则插入元素e为新的队尾，返回SUCCESS
{
	if ((rear + 1) % maxSize == front)
