...
//	辅助函数:
	...
	void InExpression(BinTreeNode<ElemType>*r,void (*Visit)(const ElemType &)) const;
...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	void InExpression(void (*Visit)(const ElemType &)) const;
...
template <class ElemType>
void BinaryTree<ElemType>::InExpression(BinTreeNode<ElemType> *r, void (*Visit)(const ElemType &)) const
{
	if (r != NULL)
    {
        if ((r->leftChild != NULL) && (r->data=="*"||r->data=="/") && (r->leftChild->data=="+"||r->leftChild->data=="-"))
        {
            cout << "(  ";
            InExpression(r->leftChild, Visit);
            cout << ")  ";
		}
        else
        { InExpression(r->leftChild, Visit); }
        (*Visit)(r->data);
        if ((r->rightChild != NULL) && (r->data=="*"||r->data=="/") && (r->rightChild->data=="+"||r->rightChild->data=="-"))
        {
            cout << "(  ";
            InExpression(r->rightChild, Visit);
            cout << ")  ";
		}
		else
        { InExpression(r->rightChild, Visit); }
...
template <class ElemType>
void BinaryTree<ElemType>::InExpression(void (*Visit)(const ElemType &)) const
{
    InExpression(root, Visit);
}
